...
function RecommendationsTable() {
  ...
  const getStatusBadge = (status) => {
    if (status === 'can') return <span className="status-badge status-can">Can Model</span>
    if (status === 'partial') return <span className="status-badge status-partial">Partial</span>
    return <span className="status-badge status-cannot">Cannot Model</span>
  }

  return (
    <div className="recommendations-table-wrapper">
      <table className="recommendations-table detailed expandable">
        <thead>
          <tr>
        ...
        <tbody>
          {RF_RECOMMENDATIONS_DISPLAY.map((row) => {
            ...
            return (
              <Fragment key={row.id}>
                <tr
                  className={`rec-row ${isExpanded ? 'expanded' : ''}`}
                  onClick={() => toggleRow(row.id)}
                >
                  <td className="rec-id">{row.id}</td>
                  <td className="rec-name">{row.recommendation}</td>
                  <td className="rec-status">{getStatusBadge(row.status)}</td>
                  <td className="rec-expand">
                    <span className={`expand-icon ${isExpanded ? 'open' : ''}`}>▶</span>
                  </td>
                </tr>
                {isExpanded && (
                  <tr className="rec-details-row">
                    <td></td>
                    <td colSpan={3} className="rec-details-cell">
                      <div className="rec-details">
                        <div className="rec-detail-section">
                          <div className="rec-detail-label">Description</div>
                          <div className="rec-detail-content">{row.description}</div>
                        </div>
                        <div className="rec-detail-section">
                          <div className="rec-detail-label">PolicyEngine UK Position</div>
